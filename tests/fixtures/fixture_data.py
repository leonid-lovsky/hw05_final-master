...
def post(user):
    ...
    return Post.objects.create(text='Тестовый текст 1', author=user, image=image)
...
def post_with_group(user, group):
    image = tempfile.NamedTemporaryFile(suffix=".jpg").name
    return Post.objects.create(text='Тестовый текст 2', author=user, group=group, image=image)
